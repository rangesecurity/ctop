...
func TestWsClientSubscribeVotes_WithAuthToken(t *testing.T) {
	envs, err := godotenv.Read("../.env")
	require.NoError(t, err)
	rpcUrl := envs["COSMOS_RPC"]
	authToken := envs["COSMOS_AUTH_TOKEN"]
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	client, err := wsclient.NewClient(rpcUrl, authToken)
	require.NoError(t, err)
	outCh, err := client.SubscribeVotes(ctx)
	require.NoError(t, err)
	defer client.UnsubscribeVotes(ctx)
